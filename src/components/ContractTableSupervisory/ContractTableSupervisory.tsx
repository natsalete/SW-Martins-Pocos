// components/ContractsTableSupervisory/ContractsTableSupervisory.tsx
...
export const ContractsTable = ({
  ...
  const handleApproveContract = async (contractId: string) => {
    try {
      const response = await fetch(`/api/supervisory/contracts/${contractId}/approve`, {
        method: 'PUT',
      });
      ...
      toast.error('Erro ao aprovar o contrato');
    }
  };
  ...
  const handleSignatureComplete = async (signature: string) => {
    ...
    try {
      const response = await fetch(`/api/supervisory/contracts/${selectedContractForSignature}/sign`, {
        method: 'PUT',
  ...
  const canSignContract = (contract: ServiceRequestWithContract) => {
    return contract.contract_status === 'approved' && 
           !contract.signatures?.some(sig => sig.signed_by === 'supervisor');
  };
            ...
            <TableRow key={contract.id}>
              ...
              <TableCell>
                <div className="flex gap-2">
                  ...
                  {showApproveButton && contract.contract_status === 'generated' && onApproveContract && (
                    <Button
                      variant="outline"
                      size="sm"
